...
class TravelApiController
{
    ...
    public function list(): void
    {
        ...
        if ($categoryId <= 0 && $categoryCode !== '') {
            $category = TravelCategory::findByCode($categoryCode);
        ...
        if (!$category) {
            $category = TravelCategory::findById($categoryId);
        }
        ...
        $items = TravelItem::listForCategory($categoryId, max(1, $limit), max(0, $offset));
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode([
            'categories' => [[
                'id' => (int)$category['id'],
