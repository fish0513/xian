...
class LiveApiController
{
    public function cover(): void
    {
        ...
        foreach ($categories as $category) {
            $items = LiveItem::listCoverByCategory((int)$category['id'], 6);
            $payload[] = [
                'id' => (int)$category['id'],
                'code' => $category['code'],
                'name' => $category['name'],
    ...
    public function list(): void
    {
        ...
        if ($categoryId <= 0 && $categoryCode !== '') {
            $category = LiveCategory::findByCode($categoryCode);
        ...
        if (!$category) {
            $category = LiveCategory::findById($categoryId);
        }
        ...
        $items = LiveItem::listForCategory($categoryId, max(1, $limit), max(0, $offset));
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode([
            'categories' => [[
                'id' => (int)$category['id'],
