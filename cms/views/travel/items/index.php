...
$buildUrl = function (int $targetPage) use ($base, $categoryId) {
    ...
    return View::e($base) . '/admin/travel/items?' . http_build_query($params);
};
?>
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">潮玩内容</h1>
            <p class="mt-1 text-sm text-gray-500">管理潮玩启东内容列表</p>
        </div>
        <a href="<?php echo View::e($base); ?>/admin/travel/items/create" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 shadow-primary-500/30 transition-colors">
    ...
    <div class="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center gap-4">
            <form method="get" action="<?php echo View::e($base); ?>/admin/travel/items" class="flex items-center gap-3">
                    ...
                    <?php foreach ($items as $i): ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900"><?php echo View::e($i['title']); ?></div>
                                <?php if (!empty($i['subtitle'])): ?>
                                    <div class="text-xs text-gray-500"><?php echo View::e($i['subtitle']); ?></div>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="<?php echo View::e($base); ?>/admin/travel/items/edit?id=<?php echo View::e((string)$i['id']); ?>" class="text-primary-600 hover:text-primary-900 flex items-center gap-1">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                        编辑
                                    </a>
                                    <form method="post" action="<?php echo View::e($base); ?>/admin/travel/items/delete" class="inline-block" onsubmit="return confirm('确定要删除这条内容吗？');">
